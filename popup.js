...
function load() {
  ...
  var a3_tt = localStorage['a3_tt'] || DEFAULT_A3_TT;
  $('a3_tt').value = a3_tt;
  $('a3_tt').addEventListener('input', function(evt) {
    updateEnabledStatus(3);
    if (!updateTime($('a3_tt'))) {
      evt.stopPropagation();
      return false;
    }
    localStorage['a3_tt'] = $('a3_tt').value;
    updateEnabledStatus(3);
    return true;
  }, false);
  $('a3_tt').addEventListener('change', function(evt) {
    if ($('a3_tt').value.length == 4 &&
        parseTime('0' + $('a3_tt').value)) {
      $('a3_tt').value = '0' + $('a3_tt').value;
    }
    if (!updateTime($('a3_tt'))) {
      evt.stopPropagation();
      return false;
  ...
  var a3_on = (localStorage['a3_on'] == 'true');
  $('a3_on').checked = a3_on;
  $('a3_on').addEventListener('change', function(evt) {
    window.setTimeout(function() {
      localStorage['a3_on'] = $('a3_on').checked;
      updateEnabledStatus(3);
    }, 0);
  }, false);

  var a3_ampm = localStorage['a3_ampm'] || DEFAULT_A3_AMPM;
  $('a3_ampm').selectedIndex = a3_ampm;
  $('a3_ampm').addEventListener('change', function(evt) {
    localStorage['a3_ampm'] = $('a3_ampm').selectedIndex;
  }, false);
